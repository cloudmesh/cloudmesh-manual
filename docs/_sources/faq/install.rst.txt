...
1. Virus checkers switch on that prevent the installation of programs and
   containers
2. Computers are managed by a workplace with stricter security rules.

Any of these cases are up to you to find out and solve. We have tested
CMS on an operating system that was downloaded straight from Microsoft
...
If you do not understand line by line what this does you should not
execute it Verify correctness works on Linux and OSX, I have this not
...
Save in file install-cms.sh and change values::
   ...
   #cms test # it is in your responsibility to fix this test may not work yet properly
...
Remember that `cmsd` will automatically upon –setup change the mode to
running. So if you like to switch back do so.

Please be aware there could be issues with Mongo running natively or in a
